<?php
    ...
    if(isset($_POST['rating']) && $_POST['rating']>0){
        ...
        if(isset($_COOKIE['user_cookie'])){
            ...
            $sql="INSERT INTO rating (email, item, rating, review) VALUES ('$email','frozen',$rating,'$review')";
        }
    ...
    if(isset($_COOKIE['user_cookie'])){
        ...
        /*SELECT u.first_name, r.email, r.rating, r.review FROM `rating` r, `user` u WHERE r.email = u.email and r.email = 'krutika.mude@gmail.com' AND item = 'flour'*/
        
        $query="SELECT u.first_name, r.email, r.rating, r.review FROM rating r, user u WHERE r.email=u.email AND item = 'frozen'" ;
    ...
    if(isset($_COOKIE['Product_cookie'])){
        ...
        if(in_array("Frozen",$product)){
            $index = array_search("Frozen", $product);
    ...
    function addToCart(){
        ...
        array_push($cart, "Frozen");   
        setcookie("AddToCart_cookie",serialize($cart),time()+60*60*24*5);
    }
    array_push($product, "Frozen");
    setcookie("Product_cookie",serialize($product),time()+60*60*24*5);
    setcookie('FrozenVisit', isset($_COOKIE['FrozenVisit']) ? ++$_COOKIE['FrozenVisit'] : 1,time()+60*60*24*5);
				...
				<div class="container-fluid">
					<div class="row align-items-center">
						<div class="col-md-3 col-md-offset-5 container-fluid home-about-left no-padding" style="margin-top: -140px;">
							<img class="mx-auto d-block img-fluid" src="img/frozen.png" alt="">
						</div>
						<div class="col-lg-6 home-about-right no-padding">
							<h1>Frozen Food Items</h1>
							<h5>
								Different Types of Frozen Foods in one pack
							</h5>
							<p>
								Frozen Items available - Frozen Rotis, Frozen Vegetables, Frozen Peas, Frozen Parathas, Frozen Surali Wali, Frozen Sabji Masalas 
							</p>
							<a class="primary-btn text-uppercase" href="cart_working.php" value="oils" onclick="addToCart();">Add to Cart</a>
							<form id="myForm" action="frozen.php" method="post" class="form-area contact-form text-right">
