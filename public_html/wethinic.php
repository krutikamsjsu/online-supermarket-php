<?php
    ...
    if(isset($_POST['rating']) && $_POST['rating']>0){
        ...
        if(isset($_COOKIE['user_cookie'])){
            ...
            $sql="INSERT INTO rating (email, item, rating, review) VALUES ('$email','wethinic',$rating,'$review')";
        }
    ...
    if(isset($_COOKIE['user_cookie'])){
        ...
        /*SELECT u.first_name, r.email, r.rating, r.review FROM `rating` r, `user` u WHERE r.email = u.email and r.email = 'krutika.mude@gmail.com' AND item = ''*/
        
        $query="SELECT u.first_name, r.email, r.rating, r.review FROM rating r, user u WHERE r.email=u.email AND item = 'wethinic'" ;
    ...
    if(isset($_COOKIE['Product_cookie'])){
        ...
        if(in_array("Wethinic",$product)){
            $index = array_search("Wethinic", $product);
     ...
     function addToCart(){
        ...
        if(isset($_COOKIE['AddToCart_cookie'])){
         ...
         array_push($cart, "Wethinic");   
         setcookie("AddToCart_cookie",serialize($cart),time()+60*60*24*5);
     }
    setcookie("Product_cookie",serialize($product),time()+60*60*24*5);
    setcookie('WethinicVisit', isset($_COOKIE['WethinicVisit']) ? ++$_COOKIE['WethinicVisit'] : 1,time()+60*60*24*5);
?>
				...
				<div class="container-fluid">
					<div class="row align-items-center">
						<div class="col-md-3 col-md-offset-5 container-fluid home-about-left no-padding" style="margin-top: -140px;">
							<img class="mx-auto d-block img-fluid" src="img/wethinic.jpg" alt="">
						</div>
						<div class="col-lg-6 home-about-right no-padding">
							<h1>Womens Ethinic</h1>
							<h5>
								Different Types of womens ethinic attires are available here
							</h5>
							<p>
								Lenghas,Gagrahs,sarees....etc
							</p>
					<a class="primary-btn text-uppercase"  href="cart_working.php" value="ondemand">Add to Cart</a>
							<form id="myForm" action="wethinic.php" method="post" class="form-area contact-form text-right">
